...
class FAIndividual:
    ...
    def calculateFitness(self):
        ...
        for tmp in self.gene:
            ...
            count=0
            while choose==-1:
                try_choose=chooseNearestRobot(getDistancesFromIndex(tmp),robot_loc,busy)
                if robot_time[try_choose]+getDistancesFromIndex(tmp)[robot_loc[try_choose]]<getTimeFromIndex(tmp)+1000:
                    if robot_time[try_choose] + getDistancesFromIndex(tmp)[robot_loc[try_choose]]<getTimeFromIndex(tmp):
                        robot_time[try_choose]=getTimeFromIndex(tmp)
                    else:
                        robot_time[try_choose]+=getDistancesFromIndex(tmp)[robot_loc[try_choose]]
                    nearest_beath=chooseNearestBerth(getDistancesFromIndex(tmp))
                    robot_loc[try_choose]=nearest_beath
                    robot_time[try_choose] += getDistancesFromIndex(tmp)[robot_loc[try_choose]]
                    choose=try_choose
                    value+=getValueFromIndex(tmp)
                else:
    ...
    def outputFitness(self):
        ...
        for tmp in self.gene:
            ...
            while choose == -1:
                try_choose = chooseNearestRobot(getDistancesFromIndex(tmp), robot_loc, busy)
                if robot_time[try_choose] + getDistancesFromIndex(tmp)[robot_loc[try_choose]] < getTimeFromIndex(
                        tmp) + 1000:
                    if robot_time[try_choose] + getDistancesFromIndex(tmp)[
                        robot_loc[try_choose]] < getTimeFromIndex(tmp):
                        robot_time[try_choose] = getTimeFromIndex(tmp)
                    else:
                        robot_time[try_choose] += getDistancesFromIndex(tmp)[robot_loc[try_choose]]
                    good2berth=[]
                    good2berth.append(str(tmp))
                    nearest_beath = chooseNearestBerth(getDistancesFromIndex(tmp))
                    good2berth.append(str(nearest_beath))
                    line[try_choose].append(good2berth)
                    robot_loc[try_choose] = nearest_beath
                    robot_time[try_choose] += getDistancesFromIndex(tmp)[robot_loc[try_choose]]
                    choose = try_choose
                    value += getValueFromIndex(tmp)
            ...
            if ran<self.population[i].param[1]:
                ...
                for j in range(self.sizepop):
                    ...
                    if self.population[i].fitness<self.population[j].fitness:
                        HanMing=count_common_elements(self.population[i].gene,self.population[j].gene)
                        self.population[i].gene=regenerate_list_b(self.population[j].gene, list_large, self.population[i].gene)
...
def getRandomGene(length):
    ...
    for i in range(len(sorted_subset_df)):
        Gene.append(sorted_subset_df.iloc[i].name)
    return Gene
