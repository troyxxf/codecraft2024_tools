...
class GA:
    ...
    def run(self):
        ...
        for i in range(self.M):  # 进行M代的操作
            ...
            with open("result_map1.txt", 'a') as f:
                index = np.argmax(self.fitnesses)
                result = self.pop[index]
                f.write(str(max(self.fitnesses))+"  ")
...
if __name__ == '__main__':
    ...
    with open("result_map1.txt",'r') as f:
        lines=f.readlines()
        last_line=lines[len(lines)-1]
        last_line=last_line.split(" ")
        last_line=last_line[2:]
        print(last_line)
    chromo=[int(i) for i in last_line]
    robotStatus = np.zeros((2, 10), dtype=int)  # 1层是所处港口，2层是当前可控时间，默认全部停在0
    income = 0
    ...
    for itemId in range(len(chromo)):
        if chromo[itemId] == 1:
            ...
            if tarRobotId != -1:
                ...
                robotStatus[1][tarRobotId] = minTime + t2
                income += df['value'][itemId]
                huowu=(df['x'][itemId],df['y'][itemId])
                gangkou=(tarWharfId)
                path[tarRobotId].append(huowu)
                path[tarRobotId].append(gangkou)
            else:
                print("没有选择机器人")
    with open("path.txt",'w') as fout:
        for line in path:
            lint=[str(i) for i in line]
            fout.write(" ".join(lint))
            fout.write("\n")
        # fout.write(path)
